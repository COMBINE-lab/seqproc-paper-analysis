...
def make_manifest(tmp_path: Path, fail: bool = False):
    ...
    for tool in ("alpha", "beta"):
        code = "raise SystemExit(7)" if fail and tool == "beta" else (
...
def test_execute_and_idempotent_resume(tmp_path, monkeypatch):
    ...
    events = [json.loads(line) for line in (output / "execution-log.jsonl").read_text().splitlines()]
...
def test_failed_attempt_is_preserved_and_retried(tmp_path):
    ...
    failed = next(entry for entry in schedule["entries"] if entry["condition_id"].endswith("beta"))
...
def test_concurrent_coordinator_is_refused(tmp_path):
    ...
    with execution_lock(output):
        with pytest.raises(ScheduleError, match="another coordinator"):
...
def test_publication_manifest_requires_every_runtime_file_in_artifacts(tmp_path):
    ...
    for run in manifest["runs"]:
        run["spec"]["inputs"] = [
            {"path": str(input_path), "sha256": sha256_file(input_path), "verify": False}
        ]
